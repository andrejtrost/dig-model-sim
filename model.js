...
/*jshint esversion: 6 */
/*jslint bitwise: true */
/*global setLog, setup, process, vec, isComparisonOp, modelErr, modelWarn, model */
/* Methods: common
...
console.log("Slice.val "+variable.get().name+"("+b+") ="+v.toString()+" mask:"+mask[0]+","+mask[1]);
	...
	function get() { // pass the variable info, isVar=false
		...
		const o = {isVar:false, name:v.name, range:v.range, type:sliceType, hdl:sliceHdl};
		return o;
	}
...
console.log("Error: setNext slice unsupported range!");
	...
	function emitVHD() {
		...
		if (range===-1) {return variable.get().name+"(to_integer("+index.get().name+"))";}
		if (range===low) {return variable.get().name+"("+range+")";}
...
function Var(varName) { "use strict";
 let obj = {isVar:true, name:varName, mode:"", type:{id:"sig", unsigned:true, size:0, declared:0, array:false, asize:0}, mask:[0, 0], init:[], hdl:{}}; // target:"" or "=" numtarget:0, 1,
...
// check if numeric variable
		...
		if (obj.left === null) { // unary op
			...
			lt = type(obj.left);
			rt = type(obj.right);
console.log("Op.emit "+obj.type.size+" "+lt.id+lt.size+" "+opStr+" "+rt.id+rt.size);			
				...
				if (rt.id==="num") {  // 2A
					...
					if (opStr==="&") {
						...
						if (opStr==="+" || opStr==="-") { // special for arithmetic op
							if (rt.unsigned) {
								str +=  "unsigned'(\"\" & "+obj.left.emitVHD()+") "+opStr+" "+obj.right.emitVHD();
							} else {
								str +=  "signed'(\"\" & "+obj.left.emitVHD()+") "+opStr+" "+obj.right.emitVHD();
							}
...
function Statement(t) {  "use strict";
	...
	function visit(pass, vars, block) {  // Statement.visit
        ...
        if (pass===1) { // first pass, indentify number & type of assignments, count operands
            ...
            str += obj.target.visit()+" "+obj.id+" "; // visit target, set mode=out
...
function IfStatement() {  "use strict";
	...
	function visit(pass, vars, block) {  // Statement.visit		
        ...
        if (pass===1) {
            ...
            str += obj.ifBlock.visit(pass, vars); // visit if block
            ...
            if (obj.elseBlock!==null) {				
                ...
                str += (" ".repeat(Number(block.level)))+"else\n" + obj.elseBlock.visit(pass, vars);  // visit else block
            }
        } else if (pass===2) {
...
//console.log("Block.visit "+pass+" "+obj.name);
	...
	statements.forEach(function (st) {
		...
		str += st.visit(pass, vars, obj)+"\n";  // V31 move output spaces to objects
		if (pass===1) {
